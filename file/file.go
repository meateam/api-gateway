...
func (r *Router) GetFileByID(c *gin.Context) {
	...
	populatedFile := CreateGetFileResponse(file, userFilePermission, foundPermission, user.User)
...
func (r *Router) GetSharedFiles(c *gin.Context) {
	...
	for _, permission := range permissions.GetPermissions() {
		...
		if file.GetOwnerID() != reqUser.ID {
			...
			files = append(
				files,
				CreateGetFileResponse(file, permission.GetRole().String(), userPermission, user.User),
			)
		}
	}
...
func (r *Router) GetFileAncestors(c *gin.Context) {
	...
	for i := 0; i < len(permittedAncestors); i++ {
		...
		user, err := r.userClient.GetUserByID(c.Request.Context(), &uspb.GetByIDRequest{Id: file.GetOwnerID()})
		if err != nil {
			httpStatusCode := gwruntime.HTTPStatusFromCode(status.Code(err))
			loggermiddleware.LogError(r.logger, c.AbortWithError(httpStatusCode, err))
		}
		...
		populatedPermittedAncestors = append(
			...
			CreateGetFileResponse(file, ancestorPermissionRole.role, ancestorPermissionRole.permission, user.User))
	}
...
func CreateGetFileResponse(file *fpb.File, role string, permission *ppb.PermissionObject, user *uspb.User) *GetFileByIDResponse {
	if file == nil {
		return nil
	}
